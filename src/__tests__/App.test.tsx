...
describe("App component, by using the reusable Table component,", () => {
  beforeEach(() => {
  ...
  it('shoulde be able to fetch & show users when loaded', async () => {
    render(<App />, {})

    expect(screen.getByText(/Loading\.\.\./i)).toBeInTheDocument()
    expect(await screen.findAllByTestId("data-record")).toHaveLength(DEFAULT_MAX_RECORDS_PER_PAGE)
  })
  ...
  it('should be able to search and display results', async () => {
    ...
    const userRecords = screen.getAllByTestId("data-record")
  ...
  it('should be able to show results based on url params', async function () {
    ...
    expect(screen.getByText(/Loading\.\.\./i)).toBeInTheDocument()
    expect(await screen.findAllByTestId("data-record")).toHaveLength(10)
  ...
  it('should be able to show results ascending once user clicks on the default "First Name" column header', async () => {
    render(<App />)
    ...
    const rows = screen.getAllByTestId("data-record")
  ...
  it('should be able to show results descending once user clicks on the ascending "First Name" column header', async () => {
    render(<App />)
